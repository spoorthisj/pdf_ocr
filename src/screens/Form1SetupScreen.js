...
import {
  ...
  TextareaAutosize, Checkbox, FormControlLabel, Select, MenuItem, InputLabel, FormControl
} from '@mui/material';
...
const parseExtractedText = (text) => {
  const data = {
    ...
    supplierCode: '', purchaseOrderNumber: '', detail: false,
    assembly: false,
  ...
  data.detail = /Detail/i.test(matchField('Detail')) || false;
  data.assembly = /Assembly/i.test(matchField('Assembly')) || false;
  ...
  data.faaApproved = /FAA Approved.*\[x\]/i.test(text);
  data.aog = /AOG.*\[x\]/i.test(text);
...
const mergeParsedData = (parsedList) => {
  ...
  keys.forEach((key) => {
    if (key === 'partName' || key === 'serialNumber') {
      // skip merging partName and serialNumber here (we use dropdown)
      merged[key] = ''; 
      return;
    }
    ...
    const processFiles = async () => {
      ...
      try {
        ...
        if (parsedResults.length === 0) {
          ...
          // Extract all unique non-empty Part Names from parsedResults AFTER parsing
          const allPartNames = Array.from(new Set(parsedResults.map(r => r.partName).filter(Boolean)));
          setPartNameOptions(allPartNames);
          
          const allSerialNumbers = Array.from(new Set(parsedResults.map(r => r.serialNumber).filter(Boolean)));
          setSerialNumberOptions(allSerialNumbers);
        ...
        <MenuItem key={idx} value={serial}>
          {serial}
              ...
              <Grid container spacing={2} sx={{ mt: 2 }} alignItems="center">
                {/* Detail / Assembly */}
                <Grid item>
                  <Typography variant="body1" style={{ marginRight: 8 }}>13</Typography>
                  <FormControlLabel
                ...
                {/* Full / Partial FAI */}
                <Grid item>
                  <Typography variant="body1" style={{ marginRight: 8 }}>14</Typography>
                  <FormControlLabel
                    control={
                      <Checkbox
          ...
          <Grid item xs={12} md={6}>
            <Paper sx={{ padding: 3, maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              {rawTexts.length === 0 && (
                <Typography>No extracted text to display.</Typography>
              )}
